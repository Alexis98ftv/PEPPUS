...
def plotUERE_Statistics(CorrFile, CorrData):
    ...
    PlotConf["xTicks"] = np.arange(-1, max(CorrData[CorrIdx["PRN"]]), max(CorrData[CorrIdx["PRN"]]))

    PlotConf["Grid"] = 1
    PlotConf["LineWidth"] = 0.2

    PlotConf["xData"] = {}
    PlotConf["yData"] = {}

    # Apply AxTable to use a table as X axis
    PlotConf["AxTable"] = True

    for prn in sorted(unique(CorrData[CorrIdx["PRN"]])):
        Label = 'G'+ ("%02d" % prn)
        FilterCond1 = CorrData[CorrIdx["PRN"]] == prn
        FilterCond2 = CorrData[CorrIdx["FLAG"]] == 1
        Data = CorrData[CorrIdx["SUERE"]][FilterCond1][FilterCond2]
        if len(Data) > 0:
            Max = max(Data)
            ...
            PlotConf["xData"][Label] = prn
            PlotConf["yData"][Label] = np.array([Max, p95, Rms, Min])
        else: 
            Max = 0.00
            p95 = 0.00
            Min = 0.00
